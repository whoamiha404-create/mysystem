                        ...
                        <tr key={r.id} style={{borderTop:'1px solid #f1f5f9'}}>
                          ...
                          <td style={{padding:'10px 14px',whiteSpace:'nowrap'}}>
                            ...
                            <button onClick={async()=>{
                              ...
                              try{const result=await api.deleteReceipt(r.id);if(result?.requiresApproval){toast('Delete sent to admin approval','success');window.dispatchEvent(new Event('rentpro-change-requests-updated'));return;}setHistory(h=>h.filter(x=>x.id!==r.id));toast(tx.deleted,'info');}
                              catch(e){toast('Error: '+e.message,'error');}
